...
class Chatbot:
    ...
    def parseArgs(args):
        ...
        datasetArgs.add_argument('--collectionNames', type=str, default=[], nargs='+', help='mongo database name')
    ...
    def main(self, args=None):
        ...
        self.sess = tf.Session(config = cfg)  # TODO: Replace all sess by self.sess (not necessary a good idea) ?
    ...
    def daemonPredict(self, sentence):
        ...
        if f1:
            s0 = "sorry"            
        if 0==np.random.randint(8):  # 聊天机器人有1/6的概率随机附加一条内容，以此来增加多样性
        ...
        if 0==np.random.randint(8):  # 聊天机器人有1/4的概率后面随机附加一条内容，以此来增加多样性
            sr = self.textData.sequence2str(self.randomResponse(), clean=True)
            if not self.textData.isDirtySentence(s0):
